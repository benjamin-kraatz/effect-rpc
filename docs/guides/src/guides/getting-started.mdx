...
# Getting Started: Your First Epic RPC-Less Adventure

Alright, my friend, it's time to embark on an epic adventure. We're going to build our very first RPC-Less application. It's going to be a simple "todo" application, but don't let that fool you. This is where the magic happens. This is where you'll learn the fundamentals of RPC-Less, and this is where you'll lay the foundation for all of your future RPC-Less projects. By the end of this guide, you'll have a solid understanding of how RPC-Less works, and you'll be ready to start building your own amazing applications.
...
Now for the most important part of our quest. We're going to define the API for our todo application. This is the heart of the dragon, the source of all our power. With RPC-Less, you define your API as a simple TypeScript interface. This interface is the single source of truth for your entire application, and it's what makes RPC-Less so powerful and easy to use.
...
const app = createServer({
  ...
  router: {
    getTodo: async ({ input }) => {
      ...
      return {
        id: input.id,
        title: 'Conquer the world with RPC-Less',
        completed: false,
      }
    },
...
Here, we're using the `createServer` function from the `@rpc-less/server` package to create a new server. We're passing our API definition and a router that implements the API methods. The router is just a plain old JavaScript object where the keys are the names of our API methods and the values are the functions that implement them.
...
Here, we're using the `createClient` function from the `@rpc-less/client` package to create a new client. We're passing our API definition and a transport that specifies the URL of our server. The transport is what handles the communication between the client and the server. It's like a magical messenger owl, carrying our requests and responses back and forth.
...
Congratulations, my friend! You've just completed your first epic RPC-Less adventure. You've learned how to define an API, create a server, create a client, and use the client to call your API. You're well on your way to becoming an RPC-Less legend.

Now that you've got the basics down, you're ready to continue your quest. In the next section, we'll take a closer look at the core concepts of the library. So, grab a potion of healing, and let's continue our adventure. The world of RPC-Less awaits!
