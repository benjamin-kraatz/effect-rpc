...
Welcome back, my friend! In this section, we're going to talk about routing. Routing is like the GPS of your application. It's what determines how your application responds to different requests. With RPC-Less, you have two ways to handle routing: file-based routing and programmatic routing. Let's take a look at both.

## File-based Routing: The Scenic Route

File-based routing is the easiest way to handle routing in RPC-Less. It's like taking the scenic route on a road trip. You just create a directory of files, and each file corresponds to an API method. It's simple, it's intuitive, and it's a great way to get started with routing.
...
In this example, we're exporting the input and output schemas of the method, as well as a default function that implements the method. RPC-Less will automatically pick up this file and create a `getTodo` method on your API.
...
To create a router programmatically, you use the `createRouter` function from the `@rpc-less/server` package. The `createRouter` function takes an object where the keys are the names of the API methods and the values are functions that implement the methods.

<CodeBlock language="typescript">{`import { createRouter } from '@rpc-less/server'
import { api } from './api'
