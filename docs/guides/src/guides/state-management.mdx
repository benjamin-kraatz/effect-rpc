...
Welcome back, my friend! In this section, we're going to talk about state management. State management is one of the most challenging parts of building a web application. It's like trying to tame a wild beast. But don't worry, with Effect RPC and Zustand, we can tame the state monster together.
...
There are a lot of state management libraries out there, but our favorite is [Zustand](https://github.com/pmndrs/zustand). Zustand is a small, fast, and scalable state management library that's a perfect match for Effect RPC. It's simple, it's intuitive, and it's a joy to use.
...
<CodeBlock language="typescript">{`import { useStore } from '../store'
